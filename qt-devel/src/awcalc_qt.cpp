...
awcalc_qt::awcalc_qt()
    : QMainWindow( 0, "awcalc_qt", WDestructiveClose )
{
...
void awcalc_qt::newDoc()
{
    awcalc_qt *ed = new awcalc_qt;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void awcalc_qt::choose()
{
...
void awcalc_qt::load( const QString &fileName )
{
    QFile f( fileName );
...
void awcalc_qt::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
